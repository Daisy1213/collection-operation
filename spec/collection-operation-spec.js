...
describe('collection operation', function () {
    ...
    test('查询教师所有的单位中不重复的Depart列', () => {
        ...
        const actual = _.uniq(_.map(teachers, 'depart'));
        expect(actual).to.deep.equalInAnyOrder(expected);
    });
    ...
    test('查询Student中“95031”班或性别为“女”的同学记录', () => {
        ...
        const actual = _.filter(students, (n) => n.class === 95031 || n.ssex === '女');
        expect(actual).to.deep.equalInAnyOrder(expected);
    });
    ...
    test('以Class降序查询Student的所有记录', () => {
        ...
        const actual = _.orderBy(students, 'class', 'desc');
        expect(actual).to.deep.equalInAnyOrder(expected);
    });
    ...
    test('以Cno升序、Degree降序查询Score的所有记录', () => {
        ...
        const actual = _.orderBy(scores, ['cno', 'degree'], ['asc', 'desc']);
        expect(actual).to.deep.equalInAnyOrder(expected);
    });
    ...
    test('查询Score中的最高分的学生学号和课程号', () => {
        ...
        const maxDegree = _.maxBy(scores, 'degree').degree;
        const maxScores = _.filter(scores, (n) => n.degree === maxDegree);

        const actual = _.map(maxScores, (n) => ({
            sno: n.sno,
            cno: n.cno
        }));
        expect(actual).to.deep.equalInAnyOrder(expected);
    ...
    test('查询Student中每个学生的姓名和年龄', () => {
        ...
        const actual = students.map(student => {
            const birthYear = student.sbirthday.split('-')[0];
            return {
                sname: student.sname,
                age: year - birthYear
            };
    ...
    test('查询“男”教师及其所上的课程', () => {
        const expected = [
            {tno: 804, tname: '李诚', tsex: '男', tbirthday: '1958-12-02', prof: '副教授', depart: '计算机系', cname: '操作系统'},
            {tno: 856, tname: '张旭', tsex: '男', tbirthday: '1969-03-12', prof: '讲师', depart: '电子工程系', cname: '数据电路',},
        ];
    ...
    test('查询和“李军”同性别并同班的同学Sname.', () => {
        ...
        const studentOfLiJun = students.find(student => student.sname === '李军');
        const actual = students.filter(student => student.ssex === studentOfLiJun.ssex && student.class === studentOfLiJun.class)
                               .map(student => ({
                                   sname: student.sname
                               }));
    ...
    test('查询所有选修“计算机导论”课程的“男”同学的成绩', () => {
        const expected = [
            {ssex: '男', sno: 103, cno: '3-105', degree: 92},
            {ssex: '男', sno: 105, cno: '3-105', degree: 88},
            {ssex: '男', sno: 101, cno: '3-105', degree: 64},
            {ssex: '男', sno: 108, cno: '3-105', degree: 78},
        ];
